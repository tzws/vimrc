...
if has("win32unix")
  let s:install_dir = '"' . expand("~/.vim/bundle/jslint.vim/ftplugin/javascript") . '"'
elseif has("win32")
  " let s:install_dir = '"' . expand("~/vimfiles/ftplugin/javascript") . '"'
  let s:install_dir = '"' . expand("~/vimfiles/bundle/jslint.vim/ftplugin/javascript") . '"'
else
...
" handle cygwin path start with /cygdrive/
if has("win32unix")
  " Do something only in Cygwin
  " 这样还是不解决问题啊，在Cygwin 下执行node
  " node /cygdrive/cygwin64/home/zhangchunfei/.vim/bundle/jslint.vim/ftplugin/javascript/jslint/runjslint.jso
  " 结果会：
  " Error: Cannot find module 'C:\cygdrive\cygwin64\home\zhangchunfei\.vim\bundle\jslint.vim\ftplugin\javascript\jslint\runjslint.js'
  let s:cmd = "cd " . s:plugin_path . " && " . s:cmd . " /cygwin64" . s:plugin_path . "runjslint." . s:runjslint_ext
